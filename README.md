# Saldo Certo
![Capa LucasDevRJ](https://github.com/user-attachments/assets/b018c4ad-df19-400b-b16a-c88e23059dea)

![Static Badge](https://img.shields.io/badge/LucasDevRJ%20-%20Desenvolvedor?style=for-the-badge&label=Desenvolvedor)
![Static Badge](https://img.shields.io/badge/Finalizado%20-%20Autoral?style=for-the-badge&label=Status)
![Static Badge](https://img.shields.io/badge/11/2025%20-%2011/2024?style=for-the-badge&label=Data)

## Descrição
Sistema bancário com as funcionalidades de criar conta, consultar saldo, transferir valor, receber valor e finalizar o programa. Também possui validações de valores inseridos pelo usuário. Sistema feito em Java com execução via terminal.

### Apresentação
Criei uma [apresentação](https://www.linkedin.com/posts/lucas-pereira-de-lima-programador_java-programaaexaeto-backend-activity-7390858440211935232-mJ-Y?utm_source=share&utm_medium=member_desktop&rcm=ACoAADjMmpUB-y8JfOCMAryu6CQAVCn5DDjE31Y) em formato de vídeo.

# :hammer: Funcionalidades do projeto
- `Funcionalidade 1`: Criar conta;
- `Funcionalidade 2`: Consultar Saldo;
- `Funcionalidade 3`: Receber Valor;
- `Funcionalidade 4`: Transferir Valor;
- `Funcionalidade 5`: Finalizar Programa;

# 📁 Acesso ao projeto
* Baixe o projeto clicando <a href="https://github.com/LucasDevRJ/SaldoCerto/archive/refs/heads/main.zip">aqui</a>
* Coloque o projeto em uma IDE que utilize a linguagem Java
* Coloque o projeto baixado na IDE
* Execute!

## ✔️ Técnicas e tecnologias utilizadas
- ``Java``

# Autor
| [<img loading="lazy" src="https://avatars.githubusercontent.com/u/95040236?v=4" width=115><br><sub>Lucas Pereira de Lima</sub>](https://github.com/LucasDevRJ) |
| :---: |
